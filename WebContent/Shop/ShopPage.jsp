...
<%@ page
	import="com.shop.model.*, com.detail.cart.CartVO ,com.detail.promotion.*"%>
...
<body>
	<div class="backgroundpng">
		<img class="backgroundpng"
			src="<%=request.getContextPath()%>/images/tooopen.jpg">
	</div>
	...
	<div class="limited container ">
		...
		<div class="row">
			<ol class="breadcrumb">
				<li><a href="<%=request.getContextPath()%>/front/shopindex.jsp"
					style="color: #fff;"><strong>首頁</strong></a></li>
				<li><a href="<%=request.getContextPath()%>/front/shopindex.jsp"
					style="color: #fff;"><strong>${(proVO.NAME!=null)?proVO.NAME:'商品區'}</strong></a></li>
		...
		<div class="row" style="background: white;">
			...
			<div class="col-xs-12 col-sm-6">
				<div style="padding: 15px;">
					<td width="200"><img
						src="<%=request.getContextPath()%>/DBPicReader?ITEMNO=${((shopVO.ITEMNO)==null)?proVO.ITEMNO:shopVO.ITEMNO}"
			...
			<div class="col-xs-12 col-sm-6">
				<div class="col-sx-12 col-sm-12"
					style="border: 1px solid #eee; height: 400px; margin-top: 10px; margin-bottom: 15px;">
					<!-- 標題 -->
					...
					<div class="row main_block_detail"
						...
						<div>
							網路會員價: <span
								style="color: #B80709; font-size: 35px; font-weight: 700;">$${((shopVO.PRICE)==null)?proVO.PRICE:shopVO.PRICE}</span>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-xs-12 col-sm-2">
							<FORM METHOD="post">
								<input type="number" value="1" id="STOCK"
									style="text-align: center; border-radius: 4px; width: 50px; height: 34px; border: 1px solid">
						</div>
						<div class="col-xs-12 col-sm-4">
							<input type="hidden" name="action"
								value="${(isPro==1)?'ADD2':'ADD'}" id="action""> <input
								type="hidden" name="ITEMNO"
								value="${((shopVO.ITEMNO)==null)?proVO.ITEMNO:shopVO.ITEMNO}"
								id="ITEMNO""> <input type="submit" id="forcatch"
								class="btn btn-sm btn-success" value="放入購物車"
								style="font-family: Microsoft JhengHei; border-radius: 3px; background-color: green; font-size: 16px;">
							</FORM>
						</div>
		...
		<div style="height: 185px;">
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						<td
							style="font-weight: 700; color: #993300; font-size: medium; font-family: 新細明體;">
							經銷商/代理商/進口商：有你真好事業股份有限公司<br> 免付費服務專線：0800-090-000<br>
							如有大額採購或經銷需求，請來電洽詢，將有業務人員與您聯絡。 <br>
						</td>
					</tr>
...
<div class="container">
	...
	<div class="col-xs-12 col-sm-10" style="width: 100%;">
		<div class="title mid-content-title">
			<strong>現正促銷</strong> <span class="pull-right"> <a
				class="title-carousel-control" href="#carousel-recently-generic"
				data-slide="prev"> <i class="fa fa-angle-left"></i>
			</a> <a class="title-carousel-control" href="#carousel-recently-generic"
				data-slide="next"> <i class="fa fa-angle-right"></i>
			</a>
			</span>
		</div>
		<div id="carousel-recently-generic"
			class="carousel slide recently-added-products-page"
			data-ride="carousel">
						...
						<div class="offer offer-default silde-show-offer pull-text-center">
							<div class="detail">
								<div class="picture thumbnail"
									title="<%=((ProVO) (listforpro.get(i))).getSHOPNAME()%>">
									<a
										href="<%=request.getContextPath()%>/shop.do?action=checkone&ITEMNO=<%=((ProVO) (listforpro.get(i))).getITEMNO()%>&PRO=1"
										title="<%=((ProVO) (listforpro.get(i))).getSHOPNAME()%>"><img
										class="img-responsive" img
										src="DBPicReader?ITEMNO=<%=((ProVO) (listforpro.get(i))).getITEMNO()%>"
								...
								<div class="middle-content">
									<h2 class="product-title">
										<a
											href="<%=request.getContextPath()%>/shop.do?action=checkone&ITEMNO=<%=((ProVO) (listforpro.get(i))).getITEMNO()%>&PRO=1">
											<%=((ProVO) (listforpro.get(i))).getSHOPNAME()%></a>
									</h2>
									<div class="description"><%=((ProVO) (listforpro.get(i))).getDES().substring(0, 15)%>...
									</div>
								</div>
							</div>
							<div class="add-info">
								<div class="ash-box">
									<div class="prices">
										<span class="price old-price">原價$<%=((ProVO) (listforpro.get(i))).getOLDPRICE()%></span>
										<span class="price special-price">特價$<%=((ProVO) (listforpro.get(i))).getPRICE()%></span>
									</div>
									<div class="buttons">
										<FORM METHOD="post" ACTION="cart.do" id="btnSubmit">
											<select name="STOCK" id="STOCK">
												<c:forEach var="quantity" begin="1"
													end="<%=((ProVO) (listforpro.get(i))).getQuantity()%>"
													step="1">
													<option value="${quantity}">${quantity}
												</c:forEach>
											</select> <input type="hidden" name="action" value="ADD2" id="action">
											<input type="hidden" id="ITEMNO" name="ITEMNO"
												value="<%=((ProVO) (listforpro.get(i))).getITEMNO()%>"
												id="ITEMNO"> <input type="submit"
												class="btn btn-sm btn-success" value="放入購物車"
												style="background-color: green; font-size: 16px; padding: 5px 10px; font-family: Microsoft JhengHei; border-radius: 3px;">
										</FORM>
									</div>
								</div>
							</div>
								...
								<div class="ash-box">
									...
									<div class="buttons">
										<FORM METHOD="post" ACTION="cart.do" id="btnSubmit">
											...
											</select> <input type="hidden" name="action" value="ADD2" id="ADD2">
											<input type="hidden" id="ITEMNO" name="ITEMNO"
												value="<%=((ProVO) (listforpro.get(i))).getITEMNO()%>"
												id="ITEMNO"> <input type="submit"
												class="btn btn-sm btn-success" value="放入購物車"
...
<%@ include file="/front/css/navbar/footer.file"%>
	<style type="text/css">
.swal2-cancel {
	margin-left: 10;
	width: 100px;
