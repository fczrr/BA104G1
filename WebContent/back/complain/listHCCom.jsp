...
<%  List<HcComplainVO> list = (List<HcComplainVO>) request.getAttribute("list");
	if(list == null){
		HcComplainService hcComplainSvc = new HcComplainService();
		list = hcComplainSvc.getAll();
		...
		<div class="col-md-8">
			<div class="tab-content ">
				...
				<ul class="nav nav-tabs custom-tab">
					<li class="tapone  active"><a href="<%=request.getContextPath()%>/back/complain/listHCCom.jsp"><i class="fa fa-user-md" aria-hidden="true"></i>&nbsp長照申訴</a></li>
...
<%@ include file="page1.file" %>  
  			<c:forEach var="hcComplainVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
   					<!--將修改的那一筆加入對比色而已-->
                        <tbody id="mytablebody" ${(hcComplainVO.complainNo == param.complainNo) ? 'text-primary':''} valign='middle'>
 						...
 						<c:choose>
                            ...
                            <td class="col-text" id="showNo">${hcComplainVO.complainNo}</td>
                            <td class="col-text">${hcComplainVO.order_no}</td>
                            <td class="col-text">John Doe</td>
                            <td class="hidden-xs" id="rowWidth">${hcComplainVO.complainDetail}</td>
                            <td class="hidden-xs">${sdf.format(hcComplainVO.detailDate)}</td>
                            <c:choose>
							    <c:when test="${hcComplainVO.complainStatus == '未處理'}">
							       <td align="center" class="col-text notyetText readstat">${hcComplainVO.complainStatus}</td>
							    </c:when>
							    <c:otherwise>
							       <td class="col-text readstat">${hcComplainVO.complainStatus}</td>
							    </c:otherwise>
							</c:choose>
						
                            <td class="hidden-xs" data-toggle="modal"  data-target="#ansower">
                                <a class="btn btn-info actionReply">
                                	<input type="hidden" name="complainNo" class="complainNoDetail serchNo" disabled="disabled" value="${hcComplainVO.complainNo}">
                            ...
                            <td class="col-text" data-toggle="modal"  data-target="#moreDetail">
                                ...
                                <i class="fa fa-chevron-down"></i>
                                 <input type="hidden" name="complainNo" class="complainNoDetail" value="${hcComplainVO.complainNo}">
        ...
        <div class="modal-footer">
            <div class="btn-group btn-group-justified" role="group" aria-label="group button">
				...
				<div class="btn-group" role="group">
                    ...
                    <input type="hidden" name="complainNo" class="replyNoDe" value="${hcComplainVO.complainNo}">
                    <input type="hidden" name="whichPage"	value="<%=whichPage%>">               <!--送出當前是第幾頁給Controller-->	    
                </div>
            </div>
        </div>
...
<div class="modal fade" id="moreDetail" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
            <h3 class="modal-title detailBack" id="lineModalLabelDe"><%-- ${hcComplainVO.complainNo} --%>：<%-- ${hcComplainVO.complainStatus} --%></h3>
        </div>
        <div class="modal-body">
            ...
            <form>
              <div class="form-group">
                ...
                <p id="complainIn" class="orderNoDe"><%-- ${hcComplainVO.order_no} --%></p>
              </div>
              <div class="form-group">
                <label for="complainIntro" id="complainTi"><i class="fa fa-envelope fo-1x" aria-hidden="true"></i> 申訴內容 : </label>
                <p id="complainIn" class="compDe"><%-- ${hcComplainVO.complainDetail} --%></p>
              </div>
                <div class="form-group">
                <label for="complainIntro" id="complainTi"><i class="fa fa-clock-o" aria-hidden="true"></i> 申訴時間 : </label>
                <p id="complainIn" class="comptime"><%-- ${sdf.format(hcComplainVO.detailDate)} --%></p>
              </div>
              <hr>
              <div class="form-group">
    ...
    $(".actionReply").on('click', function() {
    	$('#ansower').remove("data-dismiss");
		  ...
		  $.ajax({
	    		 ...
	    		 url:"<%=request.getContextPath()%>/HcComplainServlet?action=getOne_For_Updat_hc&complainNo="+replyNo.val(),   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
		  ...
		  $item = $('.repComNo').html();
	    	if(detail == ""){
	  		  ...
	  		  $.ajax({
	  	    		 type:"POST",  //指定http參數傳輸格式為POST 
	  	    		 contentType:"application/x-www-form-urlencoded",
	  	    		 url:"<%=request.getContextPath()%>/HcComplainServlet?action=update&complainNo="+$item+"&complainReply="+$(".newReplay").val(),   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
//	  	    		 data:text,  //要傳給目標的data
	  	    		 dataType: "json",
    	...
    	$(".detailBack").html("訂單申訴內容");
	     ...
	     $item = $( this );
		  ...
		  $.ajax({
	    		 type:"POST",  //指定http參數傳輸格式為POST 
	    		 contentType:"application/x-www-form-urlencoded",
	    		 url:"<%=request.getContextPath()%>/HcComplainServlet?action=getOne_For_Display&complainNo="+seach.val(),   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
//	    		 data:text,  //要傳給目標的data
	    		 dataType: "json",
