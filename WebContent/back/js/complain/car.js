    ...
    $(".actionReply").on('click', function() {
    	$('#ansower').remove("data-dismiss");
		  ...
		  $.ajax({
	    		 ...
	    		 url: "/CarComplainServlet?action=getOne_For_Updat_car&complainNo="+replyNo.val()+"&emp_no="+empNo.val(),   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
		...
		console.log("送出的值"+$item);
	    	if(detail == ""){
	  		  ...
	  		  $.ajax({
	  	    		 type:"POST",  //指定http參數傳輸格式為POST 
	  	    		 contentType:"application/x-www-form-urlencoded;charset=utf-8",
	  	    		 url:path +"/CarComplainServlet?action=update&complainNo="+$item+"&complainReply="+detail,   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
//	  	    		 data:text,  //要傳給目標的data
	  	    		 dataType: "json",
