    ...
    $(".actionReply").on('click', function() {
    	$('#ansower').remove("data-dismiss");
		  ...
		  $.ajax({
	    		 ...
	    		 url:"/BA104G1/HcComplainServlet?action=getOne_For_Updat_hc&complainNo="+replyNo.val(),   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
		  ...
		  $item = $('.repComNo').html();
	    	if(detail == ""){
	  		  ...
	  		  $.ajax({
	  	    		 type:"POST",  //指定http參數傳輸格式為POST 
	  	    		 contentType:"application/x-www-form-urlencoded",
	  	    		 url:"/BA104G1/HcComplainServlet?action=update&complainNo="+$item+"&complainReply="+$(".newReplay").val(),   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
//	  	    		 data:text,  //要傳給目標的data
	  	    		 dataType: "json",
    	...
    	$(".detailBack").html("訂單申訴內容");
	     ...
	     $item = $( this );
		  ...
		  $.ajax({
	    		 type:"POST",  //指定http參數傳輸格式為POST 
	    		 contentType:"application/x-www-form-urlencoded",
	    		 url:"/BA104G1/HcComplainServlet?action=getOne_For_Display&complainNo="+seach.val(),   	 //請求目標的url，可在url內加上GET參數，如 www.xxxx.com?xx=yy&xxx=yyy
//	    		 data:text,  //要傳給目標的data
	    		 dataType: "json",
