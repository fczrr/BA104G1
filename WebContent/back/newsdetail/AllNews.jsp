 ...
 <!-- 資料欄 ================================================================================ -->
  <%@ include file="page1.file" %>  
  			<c:forEach var="newsdetailVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
   					<!--將修改的那一筆加入對比色而已-->
                        <tbody id="mytablebody" align='center' valign='middle' ${(newsdetailVO.newsno == param.newsno) ? 'bgcolor=#FF7744':''}>
 						<c:choose>
							<c:when test="${newsdetailVO.status == '下架'}">
                            ...
                            <td class="col-text showdetil" id="showNo">${newsdetailVO.newsno}</td>
                            <td class="col-text">${newsdetailVO.newstitle}</td>
                            <td class="col-text intro">${newsdetailVO.newsintro}</td>
<%--                             <td class="hidden-xs" id="rowWidth">${newsdetailVO.empno}</td> --%>
<%--                             <td class="hidden-xs"><img src="<%=request.getContextPath()%>/newsdetail/newsimgread.do?newsno=${newsdetailVO.newsno}"style= max-width:70px;max-height:70px; ></td> --%>
							<td class="hidden-xs">${sdf.format(newsdetailVO.newsdate)}</td>
                            <c:choose>
							    <c:when test="${newsdetailVO.status == '上架'}">
							       <td align="center" class="col-text notyetText readstat">${newsdetailVO.status}</td>
							    </c:when>
							    <c:otherwise>
							       <td class="col-text readstat">${newsdetailVO.status}</td>
							    </c:otherwise>
							</c:choose>
                            ...
                            <td class="col-text" data-toggle="modal"  data-target="#moreDetail">
                                <input type="submit" class="btn btn-info clickDetail" value="詳細">
                                 <input type="hidden" name="newsNo" class="complainNoDetail" value="${newsdetailVO.newsno}">
                            ...
                            <td class="hidden-xs">
	                        	...
	                        	<input type="hidden" name="newsNo" class="newsNo"  value="${newsdetailVO.newsno}">
							    <input type="hidden" name="requestURL"	value="<%=request.getServletPath()%>"><!--送出本網頁的路徑給Controller-->
							    <input type="hidden" name="whichPage"	value="<%=whichPage%>">               <!--送出當前是第幾頁給Controller-->
                            </td>
                       </tr>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h3 class="modal-title detailBack" id="lineModalLabelDe"><%-- ${newsdetailVO.complainNo} --%>：<%-- ${newsdetailVO.complainStatus} --%></h3>
        </div>
