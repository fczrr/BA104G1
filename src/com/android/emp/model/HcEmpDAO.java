...
public class HcEmpDAO implements HcEmpDAO_interface {
	...
	private static final String GET_ALL = "SELECT EMPLOYEE.EMP_NO,EMP_NAME,EXP_PRICE FROM EMPLOYEE JOIN EXPERT ON EXPERT.EMP_NO=EMPLOYEE.EMP_NO JOIN EXPERT_LIST ON EXPERT.EXP_NO=EXPERT_LIST.EXP_NO WHERE EMP_DEP='長照'";
	...
	public List<HcEmpVO> getAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				hcEmp.setPrice(rs.getInt("EXP_PRICE"));
				hcEmpList.add(hcEmp);
			...
			for (HcEmpVO hcEmpVO : hcEmpList) {
				...
				while (rs.next()) {
					expList.add(
							new ExpertVO(rs.getString("EXP_NO"), rs.getString("EXP_NAME"), rs.getString("EXP_SPEC")));
	...
	public HcWorkShiftsVO getShifts(String empNo, String monthOfYear) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				hcWorkShiftsVO = new HcWorkShiftsVO(monthOfYear, empNo, rs.getString("WORK_SHIFT_STATUS"),
						rs.getInt("TOTAL_WORK_SHIFTS"));
	...
	public List<HcWorkShiftsVO> getAllShifts(String empNo) {
		...
		try {
			...
			while (rs.next()) {
				HcWorkShiftsVO hcWorkShiftsVO = new HcWorkShiftsVO(rs.getString("MONTH_OF_YEAR"), empNo,
						rs.getString("WORK_SHIFT_STATUS"), rs.getInt("TOTAL_WORK_SHIFTS"));
	...
	private static final String GETPRICE = "SELECT EXP_PRICE FROM EXPERT_LIST JOIN EXPERT ON EXPERT.EXP_NO = EXPERT_LIST.EXP_NO WHERE EMP_NO=?";
	...
	public Integer getPrice(String empNo) {
		...
		try {
			...
			if (rs.next()) {
				price = rs.getInt("EXP_PRICE");
			}
			System.out.println("長照價格" + price);
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			if (con != null) {
